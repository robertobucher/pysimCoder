...
files = os.listdir("/home/bucher/CACSD/pysimCoder/resources/blocks/rcpBlk")
for f in files:
    f = f.rstrip("*.py")
    try:
        cmd = "from " + f + " import *"
...
from control import *
FH_5XXX_setTQ = FH_5XXX_setTQBlk([11], '/dev/pcan32',  0x01)
FH_5XXX_AD = FH_5XXX_ADBlk([1], '/dev/pcan32',  0x01,  0)
FH_5XXX_INIT = FH_5XXX_INIT_Blk('/dev/pcan32',  0x01,  0,  0, 0, 0, 0, 0, 1)
FH_5XXX_ENC = FH_5XXX_ENCBlk([2], '/dev/pcan32',  0x01,  1)
Const = constBlk([3],  D0)
Sub = sumBlk([9,3],[4],  [1,-1])
RT_Plot = scopeStream([8])
Switch = switchBlk([10,6,8],[5],  1,  0.5,  0)
Const2 = constBlk([6],  0)
Const1 = constBlk([7],  0)
Gain = matmultBlk([4],[8],  D2PHI)
LTI_discrete1 = dssBlk([1],[9],  gz,  0)
LTI_discrete = dssBlk([7,8,2],[10],  ctr,  0)
Prod = prodBlk([5,12],[11])
Step = stepBlk([12],  5,  0,  1)

blks = [FH_5XXX_setTQ,FH_5XXX_AD,FH_5XXX_INIT,FH_5XXX_ENC,Const,Sub,RT_Plot,Switch,Const2,Const1,Gain,LTI_discrete1,LTI_discrete,Prod,Step,]
fname = 'BoWRTfix'
os.chdir("./BoWRTfix_gen")
