...
\begin{figure}[htbp]	% Step
\centering
\includegraphics[width=0.9\textwidth]{defBlocks.eps}
...
\begin{figure}[htbp]	% Step
\centering
\includegraphics[width=0.9\textwidth]{xblk2Blk.eps}
...
These informations are used to generate the ``.py'' which can be modified and saved, the help file, incomplete (``.hlp'' extension``) and the `*.c* skeleton, which should be modified for the specific block tasks.

The ``.py'' and the ``.hlp'' files must be moved in the folder ``resources/blocks/rcpBlk'', the ``*.c* file must be edited and stored under ''CodeGen/devices``.
...
\subsubsection{The can generic tab}
